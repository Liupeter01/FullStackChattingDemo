...
stubpool::details::StubPool::StubPool()
		  :m_stop(false)
		  , m_queue_size(std::thread::hardware_concurrency())
		  ...
		  , m_cred(grpc::InsecureChannelCredentials())
{
		  ...
		  for (std::size_t i = 0; i < m_queue_size; ++i) {
					m_stub_queue.push(std::move(message::VerificationService::NewStub(grpc::CreateChannel(m_addr, m_cred))));
		  }
}
...
std::optional<stubpool::details::StubPool::stub_ptr>  stubpool::details::StubPool::acquire()
{
		  std::unique_lock<std::mutex> _lckg(m_mtx);
		  m_cv.wait(_lckg, [this]() {
					return !m_stub_queue.empty() || m_stop; 
		  });
