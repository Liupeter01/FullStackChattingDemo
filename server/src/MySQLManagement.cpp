...
mysql::details::MySQLManagement::MySQLManagement() noexcept
    : MySQLManagement(std::thread::hardware_concurrency() < 2
                          ? 2
...
mysql::details::MySQLManagement::MySQLManagement(
    ...
    : m_stop(false), m_timeout(timeOut), m_username(username),
      m_password(password), m_database(database), m_host(host), m_port(port) {
...
mysql::details::MySQLManagement::~MySQLManagement() { shutdown(); }
...
std::optional<mysql::details::MySQLManagement::context_ptr>
mysql::details::MySQLManagement::acquire() {
...
void mysql::details::MySQLManagement::release(
    mysql::details::MySQLManagement::context_ptr conn) {
  if (m_stop) {
    return;
  }
  std::lock_guard<std::mutex> _lckg(m_mtx);
...
mysql::MySQLRAII::MySQLRAII() : status(true) {
