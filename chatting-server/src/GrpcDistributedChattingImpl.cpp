...
::grpc::Status grpc::GrpcDistributedChattingImpl::SendFriendRequest(
          ...
          if (!session_op.has_value()) {
                    spdlog::warn("[GRPC {} Service]: Find Target User {} Error!", ServerConfig::get_instance()->GrpcServerName, request->dst_uuid());
...
::grpc::Status grpc::GrpcDistributedChattingImpl::ConfirmFriendRequest(
          ...
          * Maybe we can not find this user in the server
          */
          std::optional<std::shared_ptr<Session>> session_op = UserManager::get_instance()->getSession(std::to_string(request->src_uuid()));
          if (!session_op.has_value()) {
                    spdlog::warn("[GRPC {} Service]: Find Target User {} Error!", ServerConfig::get_instance()->GrpcServerName, request->src_uuid());
                    response->set_error(static_cast<uint8_t>(ServiceStatus::FRIENDING_TARGET_USER_NOT_FOUND));
          }
          else {
                    /*setup json data and forward to target user*/
                    ...
                    /*send a forwarding packet*/
                    session_op.value()->sendMessage(ServiceType::SERVICE_FRIENDING_ON_BIDDIRECTIONAL, root.toStyledString());
