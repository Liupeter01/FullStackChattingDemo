...
void SyncLogic::registerCallbacks()
{
          m_callbacks.insert(std::pair<ServiceType, CallbackFunc>(
                    ...
                    std::bind(&SyncLogic::handlingLogin, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3)
          ));
}
...
void SyncLogic::execute(pair &&node)
{
          ...
          try{
                    ...
                    spdlog::error("Excute Method Failed, Internel Server Error! Error Code {}", e.what());
          }
}

void SyncLogic::handlingLogin(ServiceType srv_type, std::shared_ptr<Session> session, std::unique_ptr<RecvNode<std::string>> recv)
{
          ...
          spdlog::info("[UUID = {}] Trying to login to ChattingServer with Token {}", uuid, token);
...
void SyncLogic::handlingLogout(ServiceType srv_type, std::shared_ptr<Session> session, std::unique_ptr<RecvNode<std::string>> recv)
{
