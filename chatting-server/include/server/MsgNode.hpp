...
template <typename Container> struct MsgHeader {
          ...
          MsgHeader(uint16_t id, const Container& input) noexcept
                    : _id(id)
                    , _length(input.size() + HEADER_LENGTH)
                    , _cur_length(0) {
                    _buffer.resize(_length, 0);
          }
          ...
          static constexpr std::size_t get_header_length() {
                    return HEADER_LENGTH;
          }

          typename std::iterator_traits<typename Container::iterator>::pointer get_header_base() {
                    return _buffer.data();
          }
          typename std::iterator_traits<typename Container::iterator>::pointer get_body_base() {
                    return get_header_base() + HEADER_LENGTH;
          }

          /*please be aware, it should be network sequence, please convert network sequence to host*/
          ...
          */
          ...
          bool check_body_remaining() const {
                    return !(_cur_length >= _length);
          }

private:
          typename std::iterator_traits<typename Container::iterator>::pointer get_id_base() {
                    return get_header_base();
          }
          typename std::iterator_traits<typename Container::iterator>::pointer get_length_base() {
                    return get_header_base() + sizeof(uint16_t);
          }
};

template <typename Container>
class RecvNode<Container,
          ...
          :public MsgHeader<Container>
{
          ...
          const std::size_t get_full_length () const {
                    return  this->_length;
          }
};
