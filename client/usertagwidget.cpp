...
UserTagWidget::UserTagWidget(QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::UserTagWidget)
    , m_tagSize(width(), height())
{
    ui->setupUi(this);
...
QSize UserTagWidget::sizeHint() const
{
    return m_tagSize;
}
...
{
    connect(ui->unselected_click, &OnceClickableQLabel::clicked, this, &UserTagWidget::slot_close);
}
