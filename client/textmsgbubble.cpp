...
TextMsgBubble::TextMsgBubble(ChattingRole role, const QString &text,
                             QWidget *parent)
    : m_font("Microsoft YaHei"), m_text(new QTextEdit),
...
void TextMsgBubble::addStyleSheet() {
  m_text->setStyleSheet(
      "#text_edit{color:black;background:transparent;border:none}");
}
...
void TextMsgBubble::setupTextMsg(const QString &str) {
  ...
  for (QTextBlock it = m_text->document()->begin();
       it != m_text->document()->end(); it = it.next()) {
    /*calculate the length of each line with the width of this font*/

    int txtW = int(font_settings.horizontalAdvance(it.text()));
    max_width =
        max_width < txtW ? txtW : max_width; // Update with the longest width
  }

  /*set max width*/
  // setMaximumWidth(max_width + inner_left + inner_right + 2 * text_to_border);
  setMaximumWidth(max_width + 1.5 * (inner_left + inner_right) +
                  2 * text_to_border);
