...
LoginInterface::LoginInterface(QWidget *parent)
  ...
  Tools::loadImgResources(
      ...
      ui->passwd_display->width(),
        ui->passwd_display->height());
...
void LoginInterface::registerSignal() {
  ...
  connect(this->ui->passwd_display, &PasswordDisplaySwitching::clicked, this, [this]() {
...
void LoginInterface::registerNetworkEvent() {
    connect(HttpNetworkConnection::get_instance().get(),
            ...
            &LoginInterface::signal_login_finished);
}
...
void LoginInterface::signal_login_finished(ServiceType srv_type,
                                                     QString json_data,
                                                     ServiceStatus srv_status) {
    /*handle network error*/
    if (!json_data.length() && srv_status == ServiceStatus::NETWORK_ERROR) {
        Tools::setWidgetAttribute(this->ui->status_label_3, QString("Network Error!"),
                                  false);
        return;
    }
