...
{
          spdlog::info("Loading {} ChattingServer Configrations", ServerConfig::get_instance()->ChattingServerConfig.size());
          for (const auto& server : ServerConfig::get_instance()->ChattingServerConfig) {
                    spdlog::info("[Loading {}]: {}:{}", server._name, server._host, server._port);
...
void grpc::GrpcBalancerImpl::registerUserToken(std::size_t uuid, const std::string& token)
{
          std::lock_guard<std::mutex> _lckg(token_mtx);
          tokens[uuid] = token;
}

std::optional<std::string_view> grpc::GrpcBalancerImpl::getUserToken(std::size_t uuid)
{
...
::grpc::Status grpc::GrpcBalancerImpl::addNewUserToServer(::grpc::ServerContext* context,
