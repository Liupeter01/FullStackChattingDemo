...
PictureMsgBubble::PictureMsgBubble(ChattingRole role, const QPixmap &picture,
                                   QWidget *parent)
    : m_picture(new QLabel), m_picSize(PICTURE_WIDTH, PICTURE_HEIGHT),
...
void PictureMsgBubble::setupPixmap(const QPixmap &pic) {
  ...
  m_picture->setMaximumSize(fixed.width() + left_margin + right_margin,
                            fixed.height() + 2 * up_down_margin);
}
